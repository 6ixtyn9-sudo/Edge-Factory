# Edge Factory — CLV report (2026-06-21 to 2026-07-21)

## Overall

- total unique picks: 124
- picks with at least two prices: 104
- average raw odds delta: 0.033365
- average implied-probability delta: -0.00692
- beat-later-price rate: 0.0
- beat-later-price sample: 104
- unmatched picks: 15
- picks with fewer than two snapshots: 6

## By rule

- `2way-unanimous avg_p>=65`: n=20, two_prices=16, avg_raw=0.000625, avg_ip=-0.000356, beat_rate=0.0
- `2way-unanimous avg_p>=70`: n=26, two_prices=16, avg_raw=0.0, avg_ip=0.0, beat_rate=0.0
- `3way-unanimous avg_p>=65`: n=32, two_prices=28, avg_raw=0.106786, avg_ip=-0.019107, beat_rate=0.0
- `3way-unanimous home-only avg_p>=60`: n=9, two_prices=9, avg_raw=0.0, avg_ip=0.0, beat_rate=0.0
- `3way-unanimous home-only avg_p>=65`: n=16, two_prices=16, avg_raw=0.02875, avg_ip=-0.010769, beat_rate=0.0
- `3way-unanimous min_p>=60 avg_p>=60`: n=21, two_prices=19, avg_raw=0.000526, avg_ip=-0.000351, beat_rate=0.0

## By bucket

- `CAUTION`: n=30, two_prices=29, avg_raw=0.001034, avg_ip=-0.000734, beat_rate=0.0
- `SKIPPED_VETO`: n=80, two_prices=70, avg_raw=0.049143, avg_ip=-0.009977, beat_rate=0.0
- `WATCHLIST_NO_ODDS`: n=9, two_prices=0, avg_raw=None, avg_ip=None, beat_rate=None
- `WATCHLIST_UNKNOWN_CTX`: n=5, two_prices=5, avg_raw=0.0, avg_ip=0.0, beat_rate=0.0
