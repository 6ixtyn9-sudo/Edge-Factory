...
def connect(db: str | None = None) -> duckdb.DuckDBPyConnection:
    ...
    _src_view(
        con, "forebet", f"{LOCALDATA}/forebet_*.csv.gz",
    ...
    _src_view(
        con, "zulubet", f"{LOCALDATA}/zulubet_*.csv.gz",
        common + f", {_odds('odd1')} AS odd1, {_odds('oddx')} AS oddx,"
        f" {_odds('odd2')} AS odd2, tip, league",
    )
    _src_view(
        con, "statarea", f"{LOCALDATA}/statarea_*.csv.gz",
    ...
    # 2-way consensus: forebet x zulubet (dedup to one row per match-key per day)
    con.execute("""
